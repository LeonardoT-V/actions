"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EvaluacionService = void 0;
const common_1 = require("@nestjs/common");
const evaluacion_entity_1 = require("./entities/evaluacion.entity");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
let EvaluacionService = class EvaluacionService {
    constructor(evaluacionRepository) {
        this.evaluacionRepository = evaluacionRepository;
        this.logger = new common_1.Logger('EvaluacionService');
    }
    async create(createEvaluacionDto) {
        try {
            const evaluacion = this.evaluacionRepository.create(createEvaluacionDto);
            await this.evaluacionRepository.save(evaluacion);
            return evaluacion;
        }
        catch (error) {
            console.log(error);
            if (error.code === '23505')
                throw new common_1.BadRequestException(error.detail);
            this.logger.error(error);
            throw new common_1.InternalServerErrorException('Error no esperado');
        }
    }
    findAll() {
        return this.evaluacionRepository.find({});
    }
    async findOne(id) {
        const evaluacion = await this.evaluacionRepository.findOneBy({ id });
        if (!evaluacion)
            throw new common_1.NotFoundException(`evaluacion ${id} no encontrado`);
        return evaluacion;
    }
    async update(id, updateEvaluacionDto) {
        const evaluacion = await this.evaluacionRepository.preload(Object.assign({ id: id }, updateEvaluacionDto));
        if (!evaluacion)
            throw new common_1.NotFoundException(`evaluacion ${id} no encontrado`);
        try {
            await this.evaluacionRepository.save(evaluacion);
            return evaluacion;
        }
        catch (error) {
            console.log(error);
        }
    }
    async remove(id) {
        const evaluacion = await this.findOne(id);
        await this.evaluacionRepository.remove(evaluacion);
    }
};
EvaluacionService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(evaluacion_entity_1.Evaluacion)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], EvaluacionService);
exports.EvaluacionService = EvaluacionService;
//# sourceMappingURL=evaluacion.service.js.map